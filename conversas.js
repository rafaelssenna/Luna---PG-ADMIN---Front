...
// Novo: lê o cliente do querystring (app.js envia ?client=<slug>)
...
const state = {
  ...
  // instâncias
  instances: [],
  // base pré-filtrada por cliente (se houver CLIENT_SLUG)
  baseInstances: [],         // <= NOVO
  filteredInstances: [],     // filtro de busca da UI
  instanceFilterHint: "",    // <= NOVO: ex.: "hia-clientes"
...
function handleLogin(e) {
  ...
  // Novo: antes de listar instâncias, calcula o filtro por cliente (se houver)
...
/* ===== Render: Mensagens / helpers já existentes ===== */
// (seu código de extractInteractiveMeta, buildMediaHtml, buildQuotedHtml, buildNativeFlowCard,
//  renderMessages, pickBestText, detectPlaceholders, messageDisplayText, formatTsForTranscript,
//  roleFromMessage, export progress etc. — mantidos exatamente como você enviou.)
//  >>> Para economizar espaço aqui, mantive o seu conteúdo original sem alterações.
//  >>> (Se precisar, eu te mando este bloco íntegro novamente.)
...
/* ===== Exportação (seu bloco original permanece) ===== */
// ... (mantido igual)
...
// Busca de instâncias (agora busca em cima da base pré-filtrada)
...
els.instanceList?.addEventListener("click", async (ev) => {
  ...
  const inst = (state.baseInstances.length ? state.baseInstances : state.instances).find((i) => String(i.id) === String(state.currentInstanceId));
...
// (seu bloco de export já existente; mantido)
...
function init() {
  ...
  if (autologin === "1") {
    state.isAuthenticated = true;
    goToInstances();
    // Primeiro define o filtro por cliente (se houver), depois carrega as instâncias
