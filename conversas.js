...
function getSidebarEl() {
  return document.querySelector("#screen-workspace .wa-sidebar");
}
function showSidebarOnMobile() {
  const sb = getSidebarEl();
  if (sb) sb.style.display = ""; // padrão
  state.mobileSidebarHidden = false;
}
function hideSidebarOnMobile() {
  if (!isSmallScreen()) return;
  const sb = getSidebarEl();
  if (sb) sb.style.display = "none";
  state.mobileSidebarHidden = true;
}
window.addEventListener("resize", () => {
  // se crescer para desktop, garante sidebar visível
  if (!isSmallScreen()) showSidebarOnMobile();
});
...
function handleLogin(e) {
  ...
  // validação leve (continua dummy — sem backend)
  ...
  // Carrega listas (se CORS bloquear, UI continua aberta e mostramos alerta)
...
function renderInstances() {
  const instances = state.filteredInstances.length ? state.filteredInstances : state.instances;
...
// Extrai tipo, interatividade e resposta
function extractInteractiveMeta(m) {
  ...
  // Sinais de mensagem interativa (NativeFlow, listas, botões, enquetes)
  const isInteractive =
    type.includes("nativeflow") ||
    type.includes("interactive") ||
    type.includes("poll") || // trata enquetes/“caixinha de pergunta” como interativo
  ...
  // Tenta parsear buttonParamsJSON (se veio do backend também pode estar em _buttonParamsParsed)
  const parsedParams =
    m?._buttonParamsParsed ||
    tryParseJSON(c?.buttonParamsJSON) ||
    tryParseJSON(m?.buttonParamsJSON);

  // Texto da resposta (quando usuário clica em botão/lista)
  const responseText =
  ...
  // Estrutura para card tipo Native Flow
  const nf = {
  ...
  // Se não há header/body/footer, mas o params tem infos, tenta aproveitar
...
function buildMediaHtml(m, c) {
  ...
  if (url) {
    ...
    // Vídeo detectado mas sem URL -> placeholder textual
...
function buildNativeFlowCard(nf) {
  // Placeholder para "caixinha" sem conteúdo (quando a API não envia detalhes)
  const hasContent =
    (nf && (
      nf.header ||
      nf.body ||
  ...
  els.messages.innerHTML = list
    .map((m) => {
      ...
      const rawText =
        ...
        m?.body ??
        m?.message ??
        (typeof c === "object") ? (c?.text ?? c?.caption ?? c) : c;
      ...
      // Construção da "caixinha" (NativeFlow / listas / botões) quando aplicável
      let interactiveHtml = "";
      if (inter.isInteractive) {
        interactiveHtml += buildNativeFlowCard(inter.nf);
      } else {
        // Fallback para estruturas simples (caso algum provedor mande diferente)
      ...
      // Exibe a resposta selecionada (quando houver)
      let responseHtml = "";
      if (inter.isReply) {
      ...
      // Pílulas de tipo para diferenciar visualmente (Mídia, Caixinha, Resposta)
      const pills = [];
...
async function loadInstances() {
  ...
  try {
    ...
    // feedback útil para o problema comum de CORS
    showAlert(
...
/* ===== Helpers de Exportação (TODOS os chats da instância) ===== */
...
async function fetchMessagesForChat(instanceId, chatObj) {
  ...
  params.set("chatId", candidates[0]);
  params.set("alts", candidates.slice(1).join(","));
  params.set("limit", 1000); // puxa bastante de uma vez
  params.set("all", "1");
...
// ===== Helpers para transcript TXT (apenas interação exibível) =====
...
// ===== Exporta TODOS os chats da instância em .TXT (apenas interação exibível) =====
async function exportAllChatsForInstance(instanceId) {
  // sempre carrega a lista COMPLETA (sem filtro) antes de exportar
  ...
  async function worker() {
    while (idx < chats.length && !exportAbort.cancel) {
      ...
      try {
        ...
        // Cada mensagem: "YYYY-MM-DD HH:mm - Papel: texto"
        for (const m of messages) {
          ...
          if (!text) continue; // ignora mensagens sem nada exibível
          lines.push(`${tsStr ? tsStr + " - " : ""}${role}: ${text}`);
        }

        lines.push(""); // separador entre chats
      } catch (e) {
        lines.push(`==============================`);
        lines.push(`Chat: ${chatName}`);
        if (chatId) lines.push(`ID: ${chatId}`);
...
function wireUpLogin() {
  // evita que validação nativa impeça nosso JS de rodar
...
els.instanceSearch?.addEventListener("input", (ev) => {
  ...
  if (!q) {
    ...
    state.filteredInstances = state.instances.filter((inst) => {
...
els.btnBack?.addEventListener("click", () => {
  // No mobile: se a conversa está aberta (sidebar oculta), volta para a lista de chats.
  if (isSmallScreen() && state.mobileSidebarHidden) {
    showSidebarOnMobile();
    return;
  }
  // Desktop ou lista já visível: volta para Instâncias.
  goToInstances();
});
...
els.chatList?.addEventListener("click", async (ev) => {
  ...
  // carrega mensagens usando IDs alternativos + paginação completa
  hideSidebarOnMobile();
...
els.prevPage?.addEventListener("click", () => {
  state.chatPage = Math.max(0, state.chatPage - 1);
  renderChats();
});

// Botão Exportar → exporta TODOS os chats em .TXT (com progresso)
...
function goToInstances() {
  ...
  showSidebarOnMobile(); // garante sidebar visível ao voltar
}
function goToChats() {
  state.screen = "chats";
  els.screenLogin?.classList.add("hidden");
  ...
  showSidebarOnMobile(); // no mobile, começamos mostrando a lista
}
