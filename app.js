...
// [ADD] === Quota & Agenda helpers ===
async function loadQuota(slug) {
  try {
    return await api(`/api/quota?client=${encodeURIComponent(slug)}`);
  } catch { return null; }
}
...
async function refreshQuotaUI(slug) {
  try {
    const q = await loadQuota(slug);
    if (!q) return;
    // create or update small indicators under settings card
    const host = document.querySelector('[data-role="client-settings-card"]') || document.body;
    let box = document.getElementById('quota-box');
    if (!box) {
      box = document.createElement('div');
      box.id = 'quota-box';
      box.style.marginTop = '8px';
      box.style.fontSize = '0.95rem';
      box.innerHTML = '<div><strong>Próximo envio:</strong> <span id="next-send">—</span></div>' +
                      '<div><strong>Quota restante hoje:</strong> <span id="quota-remaining">—</span></div>';
      host.appendChild(box);
    }
    const remEl = document.getElementById('quota-remaining');
    if (remEl) remEl.textContent = `${q.remaining}/${q.cap}`;
  } catch {}
}
) {
  showLoading();
...
function handleProgressEvent(evt) {
  // [ADD] Planejamento do dia
  if ((evt.type || "item") === "schedule") {
    try {
      const nextIso = Array.isArray(evt.planned) && evt.planned.length ? evt.planned[0] : null;
      const ns = document.getElementById('next-send');
      if (ns) ns.textContent = nextIso ? formatHHMM(nextIso) : "—";
      if (state && state.selected) refreshQuotaUI(state.selected);
    } catch (e) { console.debug("schedule ui err", e); }
    return;
  }
