...
function startProgressStream(slug) {
  ...
  if (!slug) return;
  const url = `${API_BASE_URL}/api/progress?client=${encodeURIComponent(slug)}`.replace(/\/\//g, '/').replace('http:/', 'http://').replace('https:/','https://');
  // Se a API está em outro host, preserva o protocolo e o domínio já presentes
  const absolute = (API_BASE_URL && API_BASE_URL.startsWith("http")) ? `${API_BASE_URL}/api/progress?client=${encodeURIComponent(slug)}` : `/api/progress?client=${encodeURIComponent(slug)}`;
  const src = new EventSource(absolute);
  progressEvtSource = src;

  // Atualiza barra ao abrir
...
function handleProgressEvent(evt) {
  ...
  // tabela (mantém últimas 200 linhas)
  progressBuffer.push({
